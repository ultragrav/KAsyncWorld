...
import net.ultragrav.kasyncworld.deserializeNBTOrdered
import net.ultragrav.kasyncworld.serializeNBT
import net.ultragrav.kasyncworld.serializeNBTOrdered
 ...
 */
object AWChunkCodecOrderedV1 : ChunkCodec {
    ...
    override fun encode(writer: DataWriter, chunk: AsyncChunk) {
        ...
        chunk.blockEntities.forEach {
            ...
            writer.writeByteArray(serializeNBTOrdered(it.value))
        }

        // Entities
        writer.writeInt(chunk.entities.size)
        chunk.entities.forEach { writer.writeByteArray(serializeNBTOrdered(it)) }

        // Persistent data
        writer.writeByteArray(serializeNBTOrdered(chunk.persistentData))
        ...
        }.forEach { writer.writeByteArray(serializeNBTOrdered(it)) }
    ...
    override fun decode(reader: DataReader, factory: AsyncChunkFactory): AsyncChunk {
        ...
        repeat(blockEntityCount) {
            ...
            val nbt = deserializeNBTOrdered(reader.readByteArray())
        ...
        chunk.persistentData = deserializeNBTOrdered(reader.readByteArray())

        // Read block and fluid ticks
        val blockTickCount = reader.readInt()
        repeat(blockTickCount) {
            val tickNbt = deserializeNBTOrdered(reader.readByteArray())
